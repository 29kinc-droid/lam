last-commit: 5213c844a55ed5665c61bd4dc91b208828e3502a
...
## 관계명 통제 어휘 (Wikidata 표준)

- `relation` 인자를 모델이 자유롭게 짓게 뒀더니 일관성이 없어서(같은 의미를 다른 단어로), Wikidata 표준 프로퍼티에서 가져온 16개 한국어 관계 어휘(`rag/relation_vocabulary.py`)로 `enum` 강제.
- 검증기(`validate_tool_call`)에 enum 체크 추가 — 목록 밖 값은 툴 인자 오류로 걸러짐.
- Neo4j에는 한국어 라벨 + Wikidata P번호를 관계 속성으로 함께 저장(사람용 가독성 + 표준 상호운용성).
- 재현 테스트: "이용현이 반려견 용순을 키웠고 노환으로 죽었다" → 모델이 정확히 `반려동물`(P1429), `사망원인`(P509)으로 나눠 저장. 확인 메시지도 정상 출력(빈 응답 재시도 효과도 확인).
- 테스트 35개 전체 통과.

## 검증기 오탐 추가 수정 + 대괄호 충돌 규칙 확립

- **실사용 중 발견**: 사용자가 이번 턴에 직접 말한 사실(예: "이용현 직업은 개발자야")을 모델이 그대로 정리해서 답했는데도, 검증기가 "근거(RAG/그래프)에 없는 내용"이라며 오탐 → `_validate_final_response`의 evidence에 **이번 턴 사용자 입력 원문**도 포함하도록 수정.
- **그래프 저장 트리거 강화**: "기억해줘" 같은 명시적 지시 없이도, 통제 어휘에 맞는 새 사실이면 먼저 저장하도록 `add_entity_relation` 설명 강화. 재현 테스트: "직업은 개발자, 연차 3개월, 월세 35만원, 대전 거주" 문장에서 모델이 스스로 4개 사실을 시도 → enum에 있는 직업/거주지는 저장, 없는 연차/월세(스칼라값)는 정확히 거부됨.
- **대괄호 충돌 버그 두 번째 발견**: enum 오류 메시지가 파이썬 리스트를 그대로 찍어서(`['a', 'b', ...]`) 모델이 이걸 답변에 인용하자 인용검증기가 또 오탐 — 첫 번째(툴 파라미터 `[string]` 타입 표기)와 같은 클래스의 버그. `validators/input_validator.py`의 enum 오류 메시지를 괄호 형식으로 변경. **규칙 확립**: 응답 텍스트에 노출될 수 있는 문자열엔 대괄호를 쓰지 않는다.
- 수정 후 재검증: 같은 시나리오가 재시도 없이 1라운드 만에 깔끔하게 처리됨. 테스트 35개 전체 통과.

## 모델 교체(qwen3:8b) + 경량 계획 단계

- **"세금 계산해줘" 같은 복잡한 시나리오를 테스트하다가** 현재 구조엔 의도파악/계획 수립 단계가 아예 없다는 게 확인됨(Phase 1에서 단순화하며 뺐던 부분). LLM 호출을 계획 전용 프롬프트로 한 번 더 하는 방식이 표준(Plan-and-Execute 패턴)이라는 걸 확인.
- **경량판으로 구현**: `ConversationLoop.plan(user_input)`을 호출자가 명시적으로 트리거(자동 판단 없음), 결과를 `self._plan_text`에 저장, 매 턴 시스템 프롬프트에 포함. 단계별 검증 LLM 호출은 없음(속도 우선).
- 재현 테스트로 확인: 슬롯필링(부족한 정보 되묻기)은 잘 작동하나, 실제 세율표 RAG 문서·전용 계산 툴이 없는 채로 진행하면 모델이 **근거 없는 세율/공제 숫자를 지어내서 텍스트로 암산**함(출력 검증기도 RAG/그래프 근거가 아예 없으면 스킵되게 설계돼 있어서 이 케이스를 못 잡음). "모르면 모른다고 답해라" 지시를 추가해봤으나 검증 전이라 효과는 미확인.
- **모델을 `qwen2.5:7b-instruct` → `qwen3:8b`로 교체**: 원래 모델의 `tools=`+특정 질문 조합 빈 응답 버그가 `qwen3:8b`에선 재현 안 됨(네이티브 `tools=`, 프롬프트 기반 방식 둘 다 확인). 2026-08 기준 외부 벤치마크(Berkeley BFCL 등)로도 이 체급에서 툴콜링 신뢰도가 더 높다고 확인. `qwen3.8`(27B~, 18GB+)이라는 별개의 최신 모델도 있었지만 이 iGPU엔 너무 커서 제외.
- **thinking 모드 끔**: `qwen3:8b`는 기본적으로 thinking이 켜져서 같은 호출이 58초→14초(`think=False`)로 4배 차이. `llm/client.py`에 `think=False` 명시.
- 디버그 라벨도 계획 블록이 시스템 프롬프트에 포함된다는 걸 명시하도록 수정(`_debug_print_turn_start`).
- 전체 테스트 35개는 `qwen3:8b` 기준으로도 통과.

## 개발 환경 특이사항

- WSL2 Ubuntu-24.04를 지운 뒤(8단계) 머신을 재부팅했더니 Docker Desktop이 `Cannot start Docker Compose application... WSL_E_DISTRO_NOT_FOUND`로 기동 실패. 원인은 Docker Desktop 설정(`%APPDATA%\Docker\settings-store.json`)의 `IntegratedWslDistros`가 여전히 삭제된 `Ubuntu-24.04`를 참조하고 있었기 때문. 설정 파일 직접 수정(빈 배열로)으로 해결, 그 과정에서 별개로 내부 VM이 멈춘 건 `wsl --shutdown` 후 재기동으로 해결. 이 프로젝트와 무관한 시스템 이슈지만, WSL 배포판을 지운 뒤에는 Docker Desktop WSL Integration 설정도 같이 정리해야 한다는 교훈.

## 남은 것 / 다음 시작점

- 웹검색 툴 백엔드 미정 (DuckDuckGo 무료 vs 유료 API) — 필요해지면 결정.
- 중국어 섞임 문제는 `qwen2.5:7b-instruct` 기준으로 발견됐던 것 — `qwen3:8b` 교체 후 재발 여부는 아직 재검증 안 함.
- 계획 단계 이후 "모르면 모른다고 답해라" 지시 추가한 것도 아직 효과 검증 전(테스트 도중 Docker Desktop 문제로 중단됨) — 재확인 필요.
- pgvector/그래프 초기 코퍼스가 테스트용 샘플뿐 — 실 데이터 소스 미정.
- GraphRAG(`GraphStore.search`)는 pgvector와 달리 관련성 임계값이 없음 — 무관한 질문에도 매칭되는 엔티티가 있으면 그래프 컨텍스트가 매번 끼어들 수 있음(지금은 evidence에 사용자 입력도 포함시켜 실질적 영향은 줄어든 상태). 필요해지면 임계값 도입 검토.
