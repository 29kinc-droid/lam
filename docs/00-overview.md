...
| 그래프 온톨로지 | `Entity{name,type}` 노드 + 고정 `RELATES_TO{label,wikidata_pid}` 관계 (동적 관계 타입은 인젝션 위험으로 배제). `relation`은 Wikidata P번호 기반 16개 한국어 어휘로 enum 강제(`rag/relation_vocabulary.py`). 상세: [`05-graphrag.md`](05-graphrag.md) |
