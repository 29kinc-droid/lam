...
def build_graph_tool(store: GraphStore) -> Tool:
    ...
    return Tool(
        ...
        description=(
            ...
            "사용자가 알려준 사람·동물·사물·개념에 대한 새로운 사실이 아래 관계 "
            "종류 중 하나에 해당하면, '기억해줘' 같은 명시적 요청이 없어도 먼저 "
            "이 툴로 저장한다 (예: 'A는 B라는 반려동물을 키운다' → 반려동물, "
            "'B는 노환으로 죽었다' → 사망원인, 'A의 직업은 개발자다' → 직업, "
            "'A는 대전에 산다' → 거주지). 한 이야기에 여러 사실이 섞여 있으면 "
            "사실마다 이 툴을 한 번씩 나눠서 여러 번 호출해라(<tool_call> 블록을 "
            "여러 개 연달아 쓰면 된다). 목록에 맞는 관계가 없으면(예: 나이, 연차, "
            "금액 같은 숫자·수량 정보) 이 툴을 쓰지 않는다. 단순 질문 응답에는 "
            "사용하지 않는다.\n\n"
